...
- **Database URL Resolution**: The backend configuration seam that accepts either a full `DATABASE_URL` or ECS-friendly database host/name/user/password components and resolves them into the SQLAlchemy async URL used by runtime, Alembic, and smoke checks.
...
- **CDK Compute Foundation**: The AWS infrastructure module that defines the backend ECR repository, ECS cluster, and API task definition before load balancing and service deployment wiring are added.
- **ECS API Task Definition**: The CDK compute seam that packages the backend image, runtime environment, database secret components, JWT secret, logging, health check, and port mapping before an ECS service attaches it to load balancing.
