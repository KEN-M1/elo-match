...
Docker image and smoke-tests `/`. The local production compose job smoke-tests the local production compose stack
and validates `compose.production-local.yaml` with Docker Compose. The CDK synth job installs infra
dependencies and runs `cdk synth`. The Postgres smoke job applies Alembic migrations against a
fresh PostgreSQL service and runs the SQLAlchemy adapter smoke path.
