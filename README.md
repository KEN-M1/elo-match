...
Docker image and smoke-tests `/`. The local production compose job validates
`compose.production-local.yaml` with Docker Compose. The CDK synth job installs infra dependencies
and runs `cdk synth`. The Postgres smoke job applies Alembic migrations against a fresh PostgreSQL
