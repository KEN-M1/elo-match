...
class LocalDatabaseToolingTests(unittest.TestCase):
    ...
    def test_deploy_api_infra_script_passes_cdk_parameters(self) -> None:
        ...
        for expected in [
            ...
            "replace-me|your-|certificate-id|123456789012|\\.example",
            "JwtSecretArn must be set to a real Secrets Manager ARN.",
            "GoogleClientId must be set to the deployed Google OAuth client ID.",
            "GoogleClientSecretArn must be set to a real Secrets Manager ARN.",
            "AllowedOrigins must be set to the deployed web origin.",
            "ApiCertificateArn must be set to a real ACM certificate ARN.",
            "ApiPublicUrl must be set to the deployed API origin.",
            "WebAppUrl must be set to the deployed web origin.",
            "WebCertificateArn must be set to a real ACM certificate ARN.",
            "Resolve-ImageTag",
            "git rev-parse --short=12 HEAD",
    ...
    def test_run_api_migrations_script_runs_one_off_ecs_task(self) -> None:
        ...
        for expected in [
            ...
            "AWS ECS could not start the migration task",
            "$runTaskResult.failures",
            "--launch-type",
            "FARGATE",
            "awsvpcConfiguration",
            "assignPublicIp",
